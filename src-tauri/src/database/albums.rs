...
pub(crate) const ALBUM_SELECT: &str = "\
WITH album_artists AS (
    SELECT album_id, GROUP_CONCAT(name, '|||') as artist_names
    FROM (
        SELECT DISTINCT t_a.album_id, ar_t.name
        FROM tracks t_a
...
impl DbHelper {
    pub fn get_all_albums(&self) -> Result<Vec<LibraryAlbum>> {
        let query = format!(
            "{} GROUP BY al.id ORDER BY al.title ASC",
            ALBUM_SELECT
        );
    ...
    pub fn get_artist_albums(&self, artist_id: i64) -> Result<Vec<LibraryAlbum>> {
        let mut stmt = self.conn.prepare(
            ...
            album_stats AS (
                SELECT album_id, COUNT(*) as track_count, COALESCE(SUM(duration_ms), 0) as total_duration_ms
                FROM tracks
                GROUP BY album_id
            )
            SELECT
                ...
                COALESCE(ast.total_duration_ms, 0) as total_duration_ms,
                aa.artist_names,
                (SELECT t.album_artist FROM tracks t WHERE t.album_id = al.id AND t.album_artist IS NOT NULL LIMIT 1) as raw_album_artist
            FROM albums al
            LEFT JOIN artists ar ON al.artist_id = ar.id
            LEFT JOIN album_stats ast ON ast.album_id = al.id
            LEFT JOIN album_artists aa ON aa.album_id = al.id
            WHERE al.id IN (
